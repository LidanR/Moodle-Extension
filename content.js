...
(function () {
	...
	let currentPageAccentColor = '#667eea';
...
>>>>>>> 25397ca4cccca7c9c31d7a52acaa34cd3c5c24f4
	...
	async function showSettingsModal() {
		...
		// Accent color presets click handler
		modal.querySelectorAll('.jct-accent-preset').forEach(preset => {
			preset.addEventListener('click', () => {
				const color = preset.getAttribute('data-color');
				document.getElementById('jct-page-accent-color').value = color;
				// Update active state
				modal.querySelectorAll('.jct-accent-preset').forEach(p => p.classList.remove('active'));
				preset.classList.add('active');
				// Update hex label
				const hexLabel = modal.querySelector('#jct-accent-hex-label');
				if (hexLabel) hexLabel.textContent = color;
			});
		});

		// Live update accent color label when picker changes
		const accentColorInput = document.getElementById('jct-page-accent-color');
		accentColorInput.addEventListener('input', () => {
			const hexLabel = modal.querySelector('#jct-accent-hex-label');
			if (hexLabel) hexLabel.textContent = accentColorInput.value;
			// Update active preset
			modal.querySelectorAll('.jct-accent-preset').forEach(p => {
				p.classList.toggle('active', p.getAttribute('data-color') === accentColorInput.value);
			});
		});
	}
